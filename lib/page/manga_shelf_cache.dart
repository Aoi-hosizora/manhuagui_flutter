  ...
  static Future<void> syncShelfCaches(BuildContext context, {void Function()? onFinish, required EventSource eventSource}) async {
    ...
    if (canDelete) {
      ...
      for (var item in toDelete) {
        ...
        EventBusManager.instance.fire(ShelfCacheUpdatedEvent(mangaId: item.mangaId, reason: UpdateReason2.deleted, source: eventSource));
      }
    }
    // >>> 更新所有新记录
    for (var item in newCaches) {
      await ShelfCacheDao.addOrUpdateShelfCache(username: AuthManager.instance.username, cache: item);
      EventBusManager.instance.fire(ShelfCacheUpdatedEvent(mangaId: item.mangaId, reason: UpdateReason2.added, source: eventSource));
    }
...
class _MangaShelfCachePageState extends State<MangaShelfCachePage> with FitSystemScreenshotMixin {
  ...
  void _updateByEvent(ShelfCacheUpdatedEvent event) async {
    if (event.reason == UpdateReason2.added && !event.source.isShelfCachePage()) {
      // 非本页引起的新增 => 显示有更新
      _isUpdated = true;
      if (mounted) setState(() {});
    }
    if (event.reason == UpdateReason2.deleted && !event.source.isShelfCachePage()) {
      // 非本页引起的删除 => 显示有更新
      _isUpdated = true;
      if (mounted) setState(() {});
    }
        ...
        children: [
          ...
          IconTextDialogOption(
            ...
            onPressed: () async {
              ...
              _removed++;
              if (mounted) setState(() {});
              EventBusManager.instance.fire(ShelfCacheUpdatedEvent(mangaId: mangaId, reason: UpdateReason2.deleted, source: EventSource.shelfCachePage));
            },
          ),
        ],
      ),
