...
class _LaterSubPageState extends State<LaterSubPage> with AutomaticKeepAliveClientMixin, FitSystemScreenshotMixin {
  ...
  void _updateByEvent(LaterUpdatedEvent event) async {
    ...
    if (event.reason == UpdateReason.updated && ((!widget.isSepPage && !event.source.isLaterPage()) || (widget.isSepPage && !event.source.isSepLaterPage()))) {
      // 非本页引起的更新 => 显示有更新
      _isUpdated = true;
      if (mounted) setState(() {});
    }
  ...
  void _showPopupMenu({required int mangaId}) {
    ...
    showPopupMenuForMangaList(
      ...
      listenerIdentifier: ListenerIdentifierBuilder.create().withListener<LaterUpdatedEvent>((ev) async {
        if (ev.mangaId != manga.mangaId) {
          return; // unreachable
        }
        var newLater = await LaterMangaDao.getLaterManga(username: AuthManager.instance.username, mid: manga.mangaId);
        if (newLater == null) {
